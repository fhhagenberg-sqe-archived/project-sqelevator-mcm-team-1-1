 ...
 * <ul><i>MockElevatorState capacity.</i> The maximum number of passengers that can fit on an
 * elevator.</ul>
 * <ul><i>MockElevatorState speed.</i> The maximum speed an elevator can travel at in feet/sec.
 * It is assumed to be the same for all elevators.</ul>
 * <ul><i>MockElevatorState acceleration.</i> The rate at which the elevator can increase or decrease
 ...
 * the same height.</ul>
 * <ul><i>Number of elevators.</i> The number of elevators in the building. MockElevatorState numbering starts
 * at zero for elevator 1.</ul>
 * 
 * <p>Some elevator behavior is inherent and cannot be controlled:
 * <ul><i>Door opening & closing.</i> MockElevatorState doors automatically open when arriving at a floor and
 ...
 * <ul><i>MockElevatorState buttons.</i> Arriving passengers press the appropriate elevator button and the
 * light is automatically turned off when the elevator arrives at the associated floor.</ul>
 * <ul><i>MockElevatorState speed and acceleration.</i> The elevator will automatically accelerate to the maximum
 ...
 * <ul><i>MockElevatorState speed.</i> The current speed of the elevator. Positive speed indicates an elevator
 * heading up while negative indicates an elevator going down.</ul>
 * <ul><i>MockElevatorState acceleration.</i> The acceleration of the elevators.</ul>
 * <ul><i>MockElevatorState button status.</i> For each elevator, which floor buttons on the elevator have
 * been pressed.</ul>
 * <ul><i>MockElevatorState position.</i> The current position of the elevator, both in feet and to
 * the closest floor. Position is measured in feet above ground floor with zero being the bottom.</ul>
 * <ul><i>MockElevatorState door status.</i> Whether the doors for an elevator are open or closed. The
 * status may also indicate a transition between open or closed.</ul>
 * <ul><i>MockElevatorState target.</i> The current floor target of the elevator as set by the controller. The
 * elevator will travel to that target and stop until directed to the next target.</ul>
 * <ul><i>MockElevatorState committed direction.</i> The current committed direction of the elevator. Elevators
 ...
 * <ul><i>MockElevatorState floor service.</i> Whether a particular elevator services a particular floor. When
 ...
 * <ul><i>MockElevatorState weight.</i> This provides the current weight of the elevator less the weight of the
 ...
 * <ul><i>MockElevatorState target.</i> Sets the floor target of a particular elevator. Typically, this target
 ...
 * next target.</ul>
 * <ul><i>MockElevatorState committed direction.</i> Sets the current committed direction of the elevator. This
 ...
 * <ul><i>MockElevatorState floor service.</i> Sets whether a particular elevator services a particular floor.
